...
def command_line(run_on_execute=True):
    if run_on_execute==True:
        cli_parser = ArgumentParser(description="""A Simple Server System Check [by: Sagara_Billy]. 
                                    \n Please use --info to get all the system summary""")
        cli_parser.add_argument("-i", "--info", action='store_true', required=False, help="showing the system information summary")
        cli_parser.add_argument("-m", "--memory", action='store_true', required=False, help="showing memory status")
        cli_parser.add_argument("-c", "--cpu", action='store_true', required=False, help="showing cpu status")
        cli_parser.add_argument("-d", "--disk", action='store_true', required=False, help="showing disk status")
        cli_parser.add_argument("-b", "--boot", action='store_true', required=False, help="showing boot uptime status")
        cli_parser.add_argument("-la", "--loadaverage", action='store_true', required=False, help="showing load average status")
        cli_parser.add_argument("-n", "--network", action='store_true', required=False, help="showing network status")
        cli_parser.add_argument("-k", "--kernel", action='store_true', required=False, help="showing kernel status")

        args = cli_parser.parse_args() 

    return args.info, args.memory, args.cpu, args.disk, args.boot, args.loadaverage, args.network, args.kernel
...
if __name__ == "__main__":
    info, mem, cpu, disk, boot, loadaverage, network, kernel = command_line(run_on_execute=True)
